...
def scrape(URL, USER, PASS, ports=None, max_retries=3, wait_for=60, keep_driver=False):
    ...
    for port in ports:
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                try:
                    el = driver.find_element(By.XPATH, '//button//*[contains(text(), "Hiển thị thêm")]')
                    el.click()
                except NoSuchElementException:
                    pass
...
def parse(html, City = 'g298085'):
    ...
    for listing in soup.select('[data-test-attribute="location-results-card"]'):
        ...
        # --- Rating ---
        rating_el = listing.select_one('[data-automation="bubbleRatingValue"] span')
        rating_val = None
        if rating_el:
            # đổi dấu , -> . để parse float
            rating_val = float(rating_el.get_text(strip=True).replace(',', '.'))
        # --- Reviews ---
        reviews_val = None
        reviews_el = listing.select_one('[data-automation="bubbleReviewCount"] span')
        if reviews_el:
            txt = reviews_el.get_text(strip=True)   # "(3.101 đánh giá)"
            txt = txt.replace("(", "").replace(")", "")
            txt = txt.split()[0]                    # "3.101"
            try:
                reviews_val = int(txt.replace(".", "").replace(",", ""))
            except ValueError:
                reviews_val = None

        link         = 'https://www.tripadvisor.com' + href if href else None
        idCity = href.split('-')[1] if href and '-' in href else None
        if City and idCity != City: continue
        id = href.split('-')[2] if href and '-' in href else None
        if id[0] != 'd':
            id = None
        typeLocation = href.split('-')[0][1:] if href and '-' in href else None
        typeLocation = typeLocation.split('_')[0]
        ...
        try:
            ...
            continue
    return listings
def crawl_all_pages(driver, max_pages=34):
        ...
        for attempt in range(1, 4):
            try:
                next_btn = WebDriverWait(driver, 6).until(
                    EC.element_to_be_clickable((By.XPATH, '//button[@aria-label="Trang tiếp theo"]'))
                )
                try:
                    next_btn.click()
                except Exception:
...
if __name__ == '__main__':
    URL = 'https://www.tripadvisor.com.vn/Search?q=%C4%90%E1%BB%8Ba+%C4%91i%E1%BB%83m+%C4%90%C3%A0+N%E1%BA%B5ng'
    USER = 'tuaDA1_beKG9'
    PASS = '1452004Tuan='
    driver = None
    try:
    ...
    try:
        results = crawl_all_pages(driver, max_pages=34)
        save_to_csv(results, 'listLocation.csv')
    finally:
        try:
            if driver:
                driver.quit()
